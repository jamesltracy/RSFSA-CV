MaxentMultiSubset.WrapperCV1TrainTestEvalAIC <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, WrapperTrainSWD, WrapperTrainPresAbsID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn..., CatVarsPrefix, MaxentArgsIn, DataSetType, Output) {
  #CatVarsPrefix <- "ROADS_CAT"
  ## Following commented lines for changing VariableSubsetIn ONLY
  #VariableSubsetsIn <- VariableSubsetsIn[1:10,] #reduce subsets
  #VarNamesC <- paste0(unlist(PermImp.df2$VarNames), collapse="-")
  #VariableSubsetsIn <- data.frame(unlist(c(VariableSubsetsIn, VarNamesC)), stringsAsFactors=FALSE)
  #str(VariableSubsetsIn)
  ...
  MaxentBaseArgsPar <- c("betamultiplier=1.0", "writebackgroundpredictions=true")
  ...
  MaxentSubsetEvalStats <- foreach(i=ivec, .combine='rbind') %dopar%  {
    ...
    #VarNames <- c("BLKDEN30CM", "CONT_DFMO", "IM_INDEX", "IMI", "TMAX_ANN", "USTALFS")
    ...
    ## Obtain permutation importance values for variables in MaxEnt model
    maxentResults.df <- data.frame(MaxentOut@results, stringsAsFactors=FALSE)
    #maxentResults.df <- data.frame(read.csv("maxentResults.csv", header = TRUE, sep=','), stringsAsFactors=FALSE)
    PermImp.df <- maxentResults.df[(6+SubsetVariableNumber+1):(5+(2*SubsetVariableNumber)+1), , drop=FALSE]
    rownames(PermImp.df) <- gsub(".permutation.importance", "", rownames(PermImp.df))
    PermImp.df2 <- data.frame(cbind(rownames(PermImp.df), PermImp.df[,1]), stringsAsFactors=FALSE)
    PermImp.df2[,2] <- as.numeric(PermImp.df2[,2])
    #str(PermImp.df2)
    colnames(PermImp.df2) <- c("VarNames", "Permutation_Importance")
    #Sort PermImp.df2 by VarNames
    PermImp.df2 <-  PermImp.df2[order( PermImp.df2$VarNames),]
    ###
    ...
    MaxentSubsetEvalL <- vector(mode = "list", length = 2)
    TestDataTypes <- c("WrapperCV1Train", "WrapperCV1Test")
    for(TestDataType in TestDataTypes) {
      #TestDataType="Train"
      if(TestDataType=="WrapperCV1Train") {
        k <- 1
      } else {
        k <- 2
      }
      ...
      MaxentSubsetEvalL[[k]][[1]] <- c(VariableNamesSel, accusum.df$EnvVarsUsed, SubsetVariableNumber, TestDataType, accusum.df$MaxTSS, accusum.df$MaxKappa, accusum.df$AUC, accusum.df$Specificity_maxTSS, accusum.df$Sensitivity_maxTSS, accusum.df$ThresholdMaxTSS, accusum.df$AICc_bg, accusum.df$NumDVars, accusum.df$NumEnVars)
      #
      MaxentSubsetEvalL[[k]][[2]] <- PermImp.df2
    }
  ...
  ###
  #MaxentSubsetEvalStats <-MaxentSubsetEvalL
  #str(MaxentSubsetEvalStats)
  EvalStatsList <- list()
  Count <- 0
  for(y in 1:2) { 
    #y=1
    for(z in 1:(length(MaxentSubsetEvalStats)/2)) {
      #z=1
      Count <- Count + 1
      EvalStatsList[Count] <- data.frame(MaxentSubsetEvalStats[,y][z]$result[[1]], stringsAsFactors=FALSE)
    }
  }
  #
  MaxentSubsetEvalStats.df <- data.frame(do.call(rbind, EvalStatsList), stringsAsFactors=FALSE)
  ...
  for(z in 1:(length(MaxentSubsetEvalStats)/2)) {
    ...
    #Count=1
    Count <- Count + 1
    #PermImpList[Count]<- data.frame(MaxentSubsetEvalStats[,y][z]$result[[2]], stringsAsFactors=FALSE)
    PermImp.df1 <- data.frame(MaxentSubsetEvalStats[,1][z]$result[[2]], stringsAsFactors=FALSE)
    colnames(PermImp.df1)[1] <- "Variable"
    VarNamesC <- paste0(unlist(PermImp.df1$Variable), collapse="-")
    PermImp.df1$VarNames <- VarNamesC
    PermImp.df1 <- PermImp.df1[,c(3,1,2)]
    PermImpList[[Count]] <- PermImp.df1
  }  
  MaxentPermImp.df <- data.frame(do.call(rbind, PermImpList), stringsAsFactors=FALSE)
  ###
  #######
  if(Output==TRUE) {
    setwd(OutDirectIn)
    ...
    if(DataSetType!="") {
      write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_WrapperCV1TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
      write.table(MaxentPermImp.df, file=paste0(Species, "MaxentPermutationImportances_WrapperCV1TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
    } else {
      write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_WrapperCV1TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
      write.table(MaxentPermImp.df, file=paste0(Species, "MaxentPermutationImportances_WrapperCV1TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
    }
    out5 <- capture.output(print(MaxentArgsIn))
    cat(out5, file=paste(Species, "MaxentArgumentsWrapperCV1TrainTest_", TotVars, "TotVars_", ".txt", sep=""), sep="\n")
  }
  #
  MaxentSubsetEvalStatsList <- list()
  MaxentSubsetEvalStatsList[[1]] <- MaxentSubsetEvalStats.df
  MaxentSubsetEvalStatsList[[2]] <- MaxentPermImp.df
  stopCluster(cl)
  registerDoSEQ()
  closeAllConnections()
  gc()
  ...
  #
  return(MaxentSubsetEvalStatsList)
}
