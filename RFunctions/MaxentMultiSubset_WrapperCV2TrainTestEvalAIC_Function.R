MaxentMultiSubset.WrapperCV2TrainTestEvalAIC <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, FinalTrainSWD, FinalTrainPresAbsID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn..., SetRunIDIn, CatVarsPrefix, MaxentArgsIn, DataSetType, Output) {
  ...
  MaxentSubsetEvalStats <- foreach(i=ivec, .combine='rbind') %dopar%  {
    ...
    head(FinalTrainSWD)
    MaxentTrainDataK <- as.data.frame(FinalTrainSWD[,VarNames])
    ...
    TestDataTypes <- c("WrapperCV2Train", "WrapperCV2Test")
    for(TestDataType in TestDataTypes) {
      #TestDataType="Train"
      if(TestDataType=="WrapperCV2Train") {
        k <- 1
      } else {
        k <- 2
      }
  ...
  if(Output==TRUE) {
    ...
    if(DataSetType!="") {
      if(SetRunIDIn!="") {
        write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_WrapperCV2TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, "_", SetRunIDIn, ".csv"), sep=",", col.names=NA)
      } else {
        write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_WrapperCV2TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, ".csv"), sep=",", col.names=NA)
      }
    } else {
      if(SetRunIDIn!="") {
        write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_WrapperCV2TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunIDIn, ".csv"), sep=",", col.names=NA)
      } else {
        write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_WrapperCV2TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, ".csv"), sep=",", col.names=NA)
      }
    }
    out5 <- capture.output(print(MaxentArgsIn))
    cat(out5, file=paste(Species, "MaxentArgumentsWrapperCV2TrainTest_", TotVars, "TotVars_", ".txt", sep=""), sep="\n")
  }
  #
  stopCluster(cl)
  registerDoSEQ()
