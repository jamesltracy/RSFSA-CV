...
function(Species, VariableNamesIn, PredictorsIn, SubsetVariableNumber, TotPres, Run, SetName, VariableSubset, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrppin, kfoldgrpain, CRS.In, OutDirectIn..., CatVarsPrefix, MaxentArgsIn, Output) {
  ...
  library(stringdist)
  #
  # Create unique filepath for the temp directory so can delete temp file rasters 
  dir.create(file.path("c:/rtemp"), showWarnings=FALSE, recursive=TRUE)
  # Set temp directory
  rasterOptions(tmpdir=file.path("c:/rtemp"))
  ...
  #
  crs.geo2 <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84")  # geographical, datum WGS84
  if(missing(CRS.In)) {CRS.In=crs.geo2}
  #
  ModelTypeA <- paste("Maxent", Subset, sep="")
  Model <- paste("Maxent for", SubsetVariableNumber, "Feature Subset")
  tail(VariableSubset)
  #VariableSubset[1,1] <- "AETT_WINQ-AQUALFS-CWD_ANN-SCAI-TMIN_SPRQ-USTEPTS"
  ...
  ##
  system.time(maxent.score <- predict(MaxentOut, predictors))
  #dev.new()
  #plot(maxent.score)
  #
  ...
  #maxent.score <- calc(maxent.score1, function(x) x/1000)
  #maxent.score1 <- raster(paste0(OutDirectIn, "/HerpRKTX10_21MaxentTop250_1ThreshCal_6Vars_Beta2.tif"))
  #unlink(paste0(Species, "Maxent", OutGridID, SubsetVariableNumber, "Vars_Beta", BetaMult, "Cal"), format = "GTiff"))
  #writeRaster(maxent.score1, paste0(Species, "Maxent", OutGridID, SubsetVariableNumber, "Vars_Betab", BetaMult), format = "GTiff", overwrite=TRUE)
  # Suppress warnings for detach below
  suppressWarnings(detach("package:terra", unload=TRUE)) # detaching terra and reloading appears to allow writeRaster to work consistently
  suppressMessages(library(terra)) 
  # Save continuous MaxEnt model projection
  setwd(OutDirectIn)
  ...
  # Calculate AICc with presence point and raster data
  MaxentAICcResults.df <- aic.maxent(occ, nparams, p=maxent.score)
  AICc <- MaxentAICcResults.df[1,1]
  # Calculate AICcp with presence point data only
  MaxentAICcResults.df2 <- aic.maxent(occ, nparams, p=NULL)
  AICcp <- MaxentAICcResults.df2[1,1]
  #
  ####################################################
  # Obtain number of parameters in Maxent model
  params <- MaxentOut@lambdas[1:(length(MaxentOut@lambdas)-4)]
  ...
  nparams <- ncol(params.df)
  ##
  # Use stringdist pkg to find longest matches of VariableNamesIn within parameters
  VarNamesInList <- as.vector(VariableNamesIn[,1])
  ParamsList <- as.vector(unlist(params.df[1,]))
  #ParamsList <- c(ParamsList, "CTI^2")
  Match_Idx = amatch(ParamsList, VarNamesInList, method = 'lcs', maxDist = Inf)
  envarslist1 <- as.list(sort(VarNamesInList[Match_Idx]))
  envarslist <- unique(envarslist1)
...
#  write.table(AICcalcs.df, sep = ",", col.names=NA, file=paste0(Species, "_AIC_Vs_K_calc_check.csv", sep=""))
  ...
  TestDataTypes <- c("Train", "Test", "Train_bgp", "Test_bgp")
  for(k in 1:4) {
    #k=1
    ...
    head(PresTestPnt.spdf)
    nrow(PresTestPnt.spdf)
    ## Extract values of presence test points from maxent model grid
    system.time(prespred.df <- data.frame(raster::extract(maxent.score, PresTestPnt.spdf)))
    head(prespred.df)
    colnames(prespred.df) <- "MaxentScore"
    nrow(prespred.df)
    # Rejoin coordinates to prespred.df
    prespred.df <- cbind(prespred.df, xy)
    prespred.sfdf <- st_as_sf(prespred.df, coords = c("coords.x1", "coords.x2"), crs = CRS.In)
    head(prespred.sfdf)
    # Write shapefile including the correct projection
    st_write(prespred.sfdf, paste0(Species, TestDataType, "TestDataType_PresenceTestMaxentVals.shp"), append=FALSE)
    ## Then absence points
    head(MaxentAbsTestData)
    # Convert point data.frame to SpatialPointsDataFrame
    # First, specify xy coordinates
    ...
    ## Extract values of presence test points from maxent model grid
    system.time(abspred.df <- data.frame(raster::extract(maxent.score, AbsTestPnt.spdf)))
    colnames(abspred.df) <- "MaxentScore"
    # Rejoin coordinates to prespred.df
    abspred.df <- cbind(abspred.df, xy)
    abspred.sfdf <- st_as_sf(abspred.df, coords = c("coords.x1", "coords.x2"), crs = CRS.In)
    head(abspred.sfdf)
    # Write shapefile including the correct projection
    st_write(abspred.sfdf, paste0(Species, TestDataType, "TestDataType_AbsenceTestMaxentVals.shp"), append=FALSE)
    #############################################################################################
    ...
    accusum <- matrix(data=NA, ncol=12, nrow=1, byrow=TRUE, dimnames=list(NULL, c("MaxTSS", "Specificity_maxTSS", "Sensitivity_maxTSS", "AUC", "MaxKappa", "ThresholdMaxTSS", "AICc_bg", "NumDVars", "NumEnVars", "EnvVarsUsed", "AICc", "AICcp")))
    ...
    colnames(MaxentSubsetEvalStats.df1) <- c("VarNames", "EnvVarsUsed", "SubsetVariableNumber", "DataType", "TotalPresPnts", "SetName", "Run", "TSS", "Kappa", "AUC", "Spec", "Sens", "ThreshMxTSS", "AICc_bg", "NumDVars", "NumEnVars", "AICc", "AICcp")
    ...
    MaxentSubsetEvalStats.df1[,7:18] <- sapply(MaxentSubsetEvalStats.df1[,7:18], function(x) as.numeric(as.character(x)))
    if(k < 3) {
      MaxentSubsetEvalL[[k]] <- MaxentSubsetEvalStats.df1
    } else if(k==3){
      MaxentSubsetEvalL[[4]] <- MaxentSubsetEvalStats.df1
    } else if(k==4) {
      MaxentSubsetEvalL[[5]] <- MaxentSubsetEvalStats.df1
    }
    #Calculate difference between training and test statistics (overfitting)
    if(k==2) {
      MaxentSubsetEvalL[[3]] <- MaxentSubsetEvalL[[2]]
      MaxentSubsetEvalL[[3]][,8:18] <- MaxentSubsetEvalL[[1]][,8:18] - MaxentSubsetEvalL[[2]][,8:18]
      MaxentSubsetEvalL[[3]][,4] <- "Diff"
    } else if(k==4) {
      MaxentSubsetEvalL[[6]] <- MaxentSubsetEvalL[[2]]
      MaxentSubsetEvalL[[6]][,8:18] <- MaxentSubsetEvalL[[4]][,8:18] - MaxentSubsetEvalL[[5]][,8:18]
      MaxentSubsetEvalL[[6]][,4] <- "Diff_bgp"
    }
  }
  #####################################################################################
  ...
  # Save calibrated MaxEnt model projection
  suppressWarnings(detach("package:terra", unload=TRUE)) # detaching terra and reloading appears to allow writeRaster to work consistently
  suppressMessages(library(terra))
  setwd(OutDirectIn)
  ...
  #     
  # Delete folder of temporary rasters
  unlink(file.path("c:/rtemp"), recursive=TRUE, force=TRUE)
  gc()
