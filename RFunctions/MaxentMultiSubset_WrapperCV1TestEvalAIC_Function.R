MaxentMultiSubset.WrapperCV1TestEvalAIC <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, WrapperTrainSWD, WrapperTrainPresAbsID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn..., CatVarsPrefix, MaxentArgsIn, Output) {
  ...
  MaxentSubsetEvalStats <- foreach(i=ivec, .combine='rbind') %dopar%  {
    ...
    #MaxentOut <- maxent(MaxentTrainDataK, WrapperTrainPresAbsID)
    #MaxentArgs <- c("betamultiplier=2.0", "threshold=false", "linear=false", "product=false", "writebackgroundpredictions=true")
    #MaxentArgs <- c("betamultiplier=2.0", "writebackgroundpredictions=true") # good
    #MaxentArgs <- c("betamultiplier=2.0", "linear=false", "product=false") # bad
    #MaxentArgs <- c("betamultiplier=2.0", "threshold=false", "product=false") # good
    #MaxentArgs <- c("betamultiplier=2.0", "threshold=false", "product=false", "writebackgroundpredictions=true") # good
    #MaxentArgs <- c("betamultiplier=2.0", "linear=false") # bad
    #
  ...
  if(Output==TRUE) {
    ...
    write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_WrapperCV1Test_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_All", Sets, "_ForRanking", SetRunID, ".csv"), sep=",", col.names=NA)
    #
    out5 <- capture.output(print(MaxentArgsIn))
    cat(out5, file=paste(Species, "MaxentArgumentsWrapperCV1Test_", TotVars, "TotVars_", ".txt", sep=""), sep="\n")
  }
  #
  stopCluster(cl)
  registerDoSEQ()
